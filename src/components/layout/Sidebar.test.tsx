...
test("renders main navigation links", () => {
  ...
  expect(
    screen.queryByRole("link", { name: "Domains" }),
  ).not.toBeInTheDocument();
  expect(
    screen.queryByRole("link", { name: "Access management" }),
  ).not.toBeInTheDocument();
  expect(
    screen.queryByRole("link", { name: "Inference" }),
  ).not.toBeInTheDocument();
  expect(
...
test("marks the active route", () => {
  ...
  expect(screen.getByRole("link", { name: "Principals" })).toHaveAttribute(
    "aria-current",
    "page",
  );
});
...
test("filters navigation when complete capabilities are available", () => {
    ...
    principalContext: {
      ...
      capabilityState: {
        ...
        capabilities: [
          ...
          { capability: "CAPABILITY_AUDIT_READ" },
        ],
      },
      warnings: [],
    },
  ...
  expect(
    screen.queryByRole("link", { name: "Principals" }),
  ).not.toBeInTheDocument();
  expect(
...
test("shows cluster navigation only for raft runtimes", () => {
  renderSidebar({
    principalContext: {
      ...
      capabilityState: {
        kind: "complete",
        capabilities: [{ capability: "CAPABILITY_CLUSTER_READ" }],
      },
      warnings: [],
      clusterRuntime: {
        engine: "static",
  ...
  expect(
    screen.queryByRole("link", { name: "Cluster" }),
  ).not.toBeInTheDocument();
});
...
test("renders session controls at the bottom", async () => {
  ...
  expect(
    screen.getByRole("link", { name: /signed in as operator/i }),
