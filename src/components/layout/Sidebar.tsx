...
const navIconPaths: Record<IconKey, string[]> = {
  ...
  spaces: ["M4 5a2 2 0 0 1 2-2h5l2 2h5a2 2 0 0 1 2 2v2H4V5Z", "M4 11h16v6a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-6Z"],
  principals: ["M9 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z", "M17 11a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5Z", "M3 20a6 6 0 0 1 12 0H3Z", "M14 18a5 5 0 0 1 7 2h-5a7 7 0 0 0-2-2Z"],
  "intelligence-access": ["M12 3a5 5 0 0 0-3 9v3h6v-3a5 5 0 0 0-3-9Z", "M8 15h8v5H8v-5Z", "M10 20h4"],
  automations: ["M12 3v3", "M12 18v3", "M3 12h3", "M18 12h3", "M6.6 6.6 8.7 8.7", "M15.3 15.3l2.1 2.1", "M17.4 6.6l-2.1 2.1", "M8.7 15.3l-2.1 2.1", "M12 16a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z"],
  semantic: ["M12 4a4 4 0 0 0-4 4c0 1.2.5 2.3 1.4A4 4 0 0 0 8 17h8a4 4 0 0 0-.3-7.6A4 4 0 0 0 12 4Z", "M9 17v3", "M15 17v3", "M9 20h6"],
  cluster: ["M6 7a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z", "M18 7a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z", "M12 23a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z", "M8.5 6l2.2 10", "M15.5 6l-2.2 10", "M8.5 5h7"],
...
function NavItemIcon({ id }: { id: string }) {
  ...
  return (
    <svg className="h-4 w-4 shrink-0" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      {paths.map((d) => (
        <path key={d} d={d} />
      ))}
    </svg>
  ...
  return (
    <aside className="flex h-full w-64 shrink-0 flex-col border-r border-slate-200 bg-slate-100 p-4 dark:border-slate-800 dark:bg-slate-900/80">
              ...
              {section.features.map((item) => (
                item.availability === "disabled" ? (
                  <span
                    key={item.route}
                    aria-disabled="true"
                    className="flex cursor-not-allowed items-center gap-2 rounded-md px-3 py-2 text-sm font-medium text-slate-400 dark:text-slate-600"
      ...
      <div className="mt-auto space-y-3 pt-8">
        <NavLink className="block rounded-lg border border-slate-300 bg-white p-3 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-950/60 dark:hover:bg-slate-900" to="/me">
          <Text as="p" size="xs" intent="subtle" className="uppercase tracking-wide">
            Session
          </Text>
          <Text as="p" size="sm" className="mt-1 text-slate-700 dark:text-slate-300">
            Signed in as <span className="font-medium text-slate-950 dark:text-slate-100">{session.username}</span>
          </Text>
          <Text as="p" size="xs" intent="muted" className="mt-1 truncate text-slate-500 dark:text-slate-500" title={session.addr}>
            {session.addr}
          </Text>
        </NavLink>
        <button
        ...
        <Button className="w-full" variant="secondary" onClick={onLogout} disabled={loggingOut}>
