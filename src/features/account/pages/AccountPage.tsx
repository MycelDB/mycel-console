...
export function AccountPage({ session, principalContext, loading = false }: AccountPageProps) {
    ...
    <section className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Panel title="Session">
          ...
          <Detail label="Principal ID" value={session.principalId} />
          <Detail label="Username" value={session.username} />
        </Panel>
        <Panel title="Access context">
          <Detail label="Discovery state" value={loading ? "loading" : state} />
          <Detail label="Effective roles" value={String(roles.length)} />
          <Detail label="Effective capabilities" value={String(capabilities.length)} />
          {warnings.length > 0 && (
...
function Detail({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <dd className="mt-1 break-words text-sm text-slate-900 dark:text-slate-100">{value || "Not reported"}</dd>
    </div>
  );
}

function TokenSection({ label, values, empty }: { label: string; values: string[]; empty: string }) {
  return (
    <div>
      <Text as="p" size="sm" className="font-medium text-slate-900 dark:text-slate-100">{label}</Text>
      {values.length === 0 ? (
        <Text intent="muted" size="sm" className="mt-2 text-slate-600 dark:text-slate-400">{empty}</Text>
      ) : (
        <div className="mt-2 flex flex-wrap gap-2">
          {values.map((value) => <span key={value} className="rounded bg-sky-50 px-2 py-1 font-mono text-xs text-sky-800 dark:bg-sky-950 dark:text-sky-200">{value}</span>)}
        </div>
      )}
    </div>
  );
...
function ConsoleLink({ to, children }: { to: string; children: ReactNode }) {
  return <Link className="rounded-md border border-slate-300 px-3 py-2 text-sm font-medium text-slate-900 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-100 dark:hover:bg-slate-800" to={to}>{children}</Link>;
}
