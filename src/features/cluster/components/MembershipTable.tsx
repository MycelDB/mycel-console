...
function StateBadge({ state }: { state: string }) {
  return (
    <span
      className={`rounded-full px-2 py-1 text-xs font-semibold ${badgeClass(state)}`}
      title={state}
    >
      {formatEnumLabel(state || "unknown", "Unknown")}
    </span>
  );
}
...
function joinedOrTtl(member: ClusterMemberInfo) {
  if (member.state === "pending")
    return member.tokenExpiresAt
      ? `expires ${formatTime(member.tokenExpiresAt)}`
      : "pending";
    ...
    >
      <div className="border-b border-slate-200 px-4 py-3 dark:border-slate-800">
        ...
        <Text size="sm" intent="muted">
          Authoritative admission state. Token hashes and plaintext tokens are
          never shown.
        </Text>
      </div>
      {members.length === 0 ? (
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
            <thead className={`bg-slate-50 text-left text-xs uppercase tracking-wide ${themeClasses.text.parts.mutedLight} dark:bg-slate-900/80 ${themeClasses.text.parts.darkMuted}`}>
              <tr>
                ...
                <TableHead className="px-4 py-3">Node ID</TableHead>
                <TableHead className="px-4 py-3">Backend address</TableHead>
                <TableHead className="px-4 py-3">Joined / TTL</TableHead>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {members.map((member) => {
                const nodeKey = encodeURIComponent(
                  member.nodeId || member.nodeName,
                );
                return (
                  <tr
                    key={`${member.nodeName}-${member.nodeId || member.tokenId || member.state}`}
                  >
                    ...
                    <td className="px-4 py-3 font-medium">
                      ...
                      {member.clusterBootstrap ? " · bootstrap" : ""}
                    </td>
                    <td className="px-4 py-3">
                      <ResourceIdText value={member.nodeId} />
                    </td>
                    <td className="px-4 py-3">
                      <ResourceIdText value={member.backendAdvertiseAddr} />
                    </td>
