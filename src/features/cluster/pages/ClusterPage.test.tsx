...
const mockedGetClusterStatus = getClusterStatus as jest.MockedFunction<
  typeof getClusterStatus
>;
const mockedGetClusterHealth = getClusterHealth as jest.MockedFunction<
  typeof getClusterHealth
>;
const mockedGetClusterRuntimeStatus =
  getClusterRuntimeStatus as jest.MockedFunction<
    typeof getClusterRuntimeStatus
  >;
const mockedGetClusterSpaceDistribution =
  getClusterSpaceDistribution as jest.MockedFunction<
    typeof getClusterSpaceDistribution
  >;
const mockedGetLocalGraphConsistency =
  getLocalGraphConsistency as jest.MockedFunction<
    typeof getLocalGraphConsistency
  >;
const mockedGetGraphConsistencyReport =
  getGraphConsistencyReport as jest.MockedFunction<
    typeof getGraphConsistencyReport
  >;
const mockedGetLocalGraphForensicExport =
  getLocalGraphForensicExport as jest.MockedFunction<
    typeof getLocalGraphForensicExport
  >;
const mockedListRaftGroups = listRaftGroups as jest.MockedFunction<
  typeof listRaftGroups
>;
const mockedLookupSpaceRoute = lookupSpaceRoute as jest.MockedFunction<
  typeof lookupSpaceRoute
>;
const mockedListActivityEvents = listActivityEvents as jest.MockedFunction<
  typeof listActivityEvents
>;
const mockedListClusterMembers = listClusterMembers as jest.MockedFunction<
  typeof listClusterMembers
>;
...
describe("ClusterPage", () => {
  beforeEach(() => {
    ...
    mockedListActivityEvents.mockResolvedValue({
      events: [],
      nextPageToken: "",
    });
    ...
    mockedGetClusterSpaceDistribution.mockResolvedValue({
      totalSpaces: 5,
      routedSpaces: 5,
      unavailableRoutes: 0,
      partitionsUsed: 3,
      partitionCount: 16,
      maxPartitionSpaces: 2,
      minPartitionSpaces: 0,
      skewRatio: 2,
      partitions: Array.from({ length: 16 }, (_, partitionId) => ({
        partitionId,
        spaceCount: partitionId < 3 ? (partitionId === 0 ? 2 : 1) : 0,
      })),
      nodes: [
        {
          nodeId: 1,
    ...
    mockedLookupSpaceRoute.mockResolvedValue({
      spaceId: "490851b9-0038-4afc-b1f0-d1bd9e829bc8",
      partitionId: 7,
      leaderNodeId: 2,
  ...
  it("renders clustered overview and tabbed cluster sections", async () => {
      ...
      peers: [
        {
          ...
          clusterId: "cluster_a",
          clusterName: "dev-cluster",
          backendAdvertiseAddr: "127.0.0.1:9093",
          state: "self",
          source: "self",
          lastSeenAt: "2026-07-15T16:12:01Z",
        },
        {
          nodeId: "node_b",
          nodeName: "node-b",
          clusterId: "cluster_a",
          clusterName: "dev-cluster",
          backendAdvertiseAddr: "127.0.0.1:9094",
          state: "active",
          source: "discovered",
          lastSeenAt: "2026-07-15T16:12:59Z",
        },
      ],
    });
    mockedListClusterMembers.mockResolvedValue({
      ...
      members: [
        {
          ...
          joinedAt: "2026-07-15T16:12:01Z",
        },
        {
          nodeName: "node-c",
          state: "pending",
          tokenId: "join_tok_1",
          tokenExpiresAt: "2026-07-15T16:42:00Z",
        },
      ],
    });
    ...
    await waitFor(() =>
      expect(mockedGetClusterStatus).toHaveBeenCalledTimes(1),
    );
    ...
    expect(
      screen.queryByRole("tab", { name: "Membership" }),
    ).not.toBeInTheDocument();

    expect(mockedListActivityEvents).toHaveBeenCalledWith({
      pageSize: 50,
      categories: ["cluster"],
    });
  ...
  it("renders raft overview, groups, and route lookup", async () => {
    ...
    mockedListRaftGroups.mockResolvedValue({
      groups: [
        {
          groupId: "system",
          kind: "system",
          ...
          readDiagnostics: {
            ...
            lastAppliedWaitMillis: 3,
          },
        },
        {
          groupId: "space-partition-7",
          kind: "partition",
          partitionId: 7,
          localNodeId: 1,
          leaderNodeId: 2,
          preferredLeaderNodeId: 2,
          replicaNodeIds: [1, 2, 3],
          health: "healthy",
          term: 1,
          commitIndex: 2,
          ...
          readDiagnostics: {
            readIndexAttempts: 2,
            readIndexSuccesses: 2,
            readIndexFailures: 0,
            readIndexTimeouts: 0,
            readIndexNoLeader: 0,
            readIndexNotLeader: 0,
            applyWaitFailures: 0,
            lastAppliedWaitMillis: 1,
          },
        },
      ],
    });
    ...
    expect(
      screen.queryByText("Raft transport summary"),
    ).not.toBeInTheDocument();
    expect(
      screen.getByText("Snapshot and compaction guidance"),
    ).toBeInTheDocument();
    expect(screen.getByText(/system@2/)).toBeInTheDocument();
    expect(
      screen.getByText(
        /Automatic production raft compaction remains off\/conservative/i,
      ),
    ).toBeInTheDocument();
    ...
    await user.type(
      screen.getByLabelText(/space id route lookup/i),
      "490851b9-0038-4afc-b1f0-d1bd9e829bc8",
    );
    await user.click(screen.getByRole("button", { name: /lookup route/i }));
    expect(await screen.findByText("7")).toBeInTheDocument();
    expect(mockedLookupSpaceRoute).toHaveBeenCalledWith({
      spaceId: "490851b9-0038-4afc-b1f0-d1bd9e829bc8",
    });
    ...
    expect(
      screen.getAllByLabelText("Apply lag: 0 (Pass)").length,
    ).toBeGreaterThan(0);
    expect(
      screen.getByLabelText("Read failures: 1 (Fail)"),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("Transport failures: 2 (Fail)"),
    ).toBeInTheDocument();
    await user.click(screen.getAllByRole("button", { name: "Expand" })[0]);
    expect(screen.getByText("Snapshot")).toBeInTheDocument();
    expect(screen.getByText("Transport targets")).toBeInTheDocument();
    await user.selectOptions(
      screen.getByLabelText(/raft group status filter/i),
      "read_failures",
    );
  ...
  it("renders healthy client readiness checks", async () => {
    ...
    expect(
      screen.getAllByText("Partition groups started").length,
    ).toBeGreaterThan(0);
    expect(screen.getAllByText("Cluster ID match").length).toBeGreaterThan(0);
    expect(
      screen.getByLabelText(
        /Metadata applied: Whether committed system Raft metadata/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText(/Client readiness: Overall safe-to-serve signal/i),
    ).toBeInTheDocument();
  ...
  it("renders blocked readiness from cluster health", async () => {
    ...
    expect(
      screen.getByText("system raft metadata has not been validated"),
    ).toBeInTheDocument();
  ...
  it("runs read-only graph consistency diagnostics", async () => {
    ...
    mockedGetLocalGraphConsistency.mockResolvedValue({
      stats,
      ...
      warnings: [
        {
          ...
          severity: "critical",
          raftNodeId: 2,
          message: "replica checksum differs",
        },
      ],
    ...
    expect(
      screen.getByText(/read-only latest-state evidence/i),
    ).toBeInTheDocument();
    await user.type(screen.getByLabelText(/consistency space id/i), "sp_main");
    await user.type(
      screen.getByLabelText(/consistency domain id/i),
      "dom_default",
    );
    await user.click(screen.getByRole("button", { name: /run local check/i }));
    expect(
      await screen.findByText("Local latest-state check"),
    ).toBeInTheDocument();
    expect(screen.getByText("graph")).toBeInTheDocument();
    expect(mockedGetLocalGraphConsistency).toHaveBeenCalledWith({
      spaceId: "sp_main",
      domainId: "dom_default",
    });

    await user.click(
      screen.getByRole("button", { name: /run cluster report/i }),
    );
    expect(
      await screen.findByText("Cluster consistency report"),
    ).toBeInTheDocument();
    expect(screen.getByText("Divergent")).toBeInTheDocument();
    expect(screen.getByText(/CHECKSUM_MISMATCH/)).toBeInTheDocument();
    expect(screen.getByText("unreachable")).toBeInTheDocument();
    expect(mockedGetGraphConsistencyReport).toHaveBeenCalledWith({
      spaceId: "sp_main",
      domainId: "dom_default",
    });

    await user.click(
      screen.getByRole("button", { name: /run forensic export/i }),
    );
    expect(
      await screen.findByText("Local forensic export"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Export response is truncated/i),
    ).toBeInTheDocument();
    expect(screen.getByText("report-1")).toBeInTheDocument();
    expect(screen.getByText("page-2")).toBeInTheDocument();
    expect(screen.getByText("forensic warning")).toBeInTheDocument();
    expect(mockedGetLocalGraphForensicExport).toHaveBeenCalledWith({
      spaceId: "sp_main",
      domainId: "dom_default",
      pageSize: 100,
      pageToken: "",
      sourceLabel: "admin-ui",
    });
  });
