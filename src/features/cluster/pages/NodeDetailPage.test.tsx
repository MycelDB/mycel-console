...
describe("NodeDetailPage", () => {
  ...
  it("renders raft responsibilities", async () => {
    mockedGetClusterRuntimeStatus.mockResolvedValue({
      ...
      raftReplicaFactor: 3,
      localRaftNodeId: 1,
      raftNodeAddrs: ["node-a:9091", "node-b:9091", "node-c:9091"],
      raftGroupCount: 17,
    ...
    expect(
      await screen.findByText("Raft responsibilities"),
    ).toBeInTheDocument();
  ...
  it("renders node details", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "node-b" }),
    ).toBeInTheDocument();
