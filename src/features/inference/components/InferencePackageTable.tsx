...
export function InferencePackageTable({ packages }: InferencePackageTableProps) {
    ...
    return (
      <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center dark:border-slate-700 dark:bg-slate-900/40">
        ...
        <Text intent="muted" size="sm" className="mt-2 text-slate-600 dark:text-slate-400">
          Import a package JSON file to install or update inference catalog definitions.
        </Text>
      </div>
    );
  }
      ...
      <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
        <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-950/40">
          <tr>
            ...
            <th className="px-4 py-3">Definitions</th>
        ...
        <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
          {packages.map((pkg) => (
            <tr key={pkg.inferencePackageId || `${pkg.name}-${pkg.version}`} className="hover:bg-slate-100 dark:hover:bg-slate-800/40">
              <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-100">{pkg.name}</td>
              <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{pkg.version}</td>
              <td className="max-w-64 truncate px-4 py-3 text-slate-700 dark:text-slate-300" title={pkg.source}>{pkg.source || "—"}</td>
              <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{formatCounts(pkg.definitionCounts)}</td>
              <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{formatInstalledAt(pkg.installedAt)}</td>
              <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{pkg.installedBy || "—"}</td>
              <td className="max-w-48 truncate px-4 py-3 font-mono text-xs text-slate-600 dark:text-slate-400" title={pkg.checksum}>{pkg.checksum || "—"}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function formatInstalledAt(value: string) {
  ...
  if (/^-?\d+(\.\d+)?$/.test(value)) {
    ...
    if (Number.isFinite(millis)) return new Date(millis).toISOString().replace("T", " ").replace(/\.\d+Z$/, " UTC");
  }
