...
function renderPage(
  overrides: Partial<Parameters<typeof InferencePage>[0]> = {},
) {
  const services = {
    listInferencePackagesService: jest
      .fn<
        Promise<ListInferencePackagesResponse>,
        [ListInferencePackagesInput | undefined]
      >()
      .mockResolvedValue(packagesResponse),
    applyInferencePackageService: jest
      .fn<Promise<ApplyInferencePackageResponse>, any>()
      .mockResolvedValue({
        package: packagesResponse.packages[0],
        modelEndpointCount: 1,
        modelCount: 2,
        vectorStoreCount: 1,
        capabilityCount: 2,
      }),
    listModelEndpointsService: jest.fn().mockResolvedValue({
      modelEndpoints: [
        {
          modelEndpointId: "ep1",
          key: "openai",
    ...
    listModelsService: jest.fn().mockResolvedValue({
      models: [
        {
          modelId: "m1",
    ...
    listVectorStoresService: jest.fn().mockResolvedValue({
      vectorStores: [
        {
          vectorStoreId: "vs1",
          key: "mycel-file",
    ...
    listModelEndpointCapabilitiesService: jest.fn().mockResolvedValue({
...
function renderAccessPage(
  ...
  return renderPage({ section: "access", ...overrides });
}
...
test("renders import history as the last tab", async () => {
  ...
  expect(screen.getByRole("tab", { name: "Endpoints" })).toHaveAttribute(
    "aria-selected",
    "true",
  );
  const tabs = screen.getAllByRole("tab").map((tab) => tab.textContent);
  expect(tabs).toEqual(["Endpoints", "Models", "Import history"]);
  expect(
    screen.queryByRole("tab", { name: "Credentials" }),
  ).not.toBeInTheDocument();
  expect(
    screen.queryByRole("tab", { name: "Vector stores" }),
  ).not.toBeInTheDocument();
...
test("renders catalog tabs and loads endpoint section", async () => {
  ...
  expect(await screen.findByText("Openai Compatible")).toBeInTheDocument();
  expect(services.listModelEndpointsService).toHaveBeenCalledWith({
    pageSize: 100,
...
test("sends catalog filter inputs", async () => {
  ...
  await waitFor(() =>
    expect(services.listModelEndpointsService).toHaveBeenLastCalledWith({
      pageSize: 100,
  ...
  await waitFor(() =>
    expect(services.listModelsService).toHaveBeenLastCalledWith({
      pageSize: 100,
    }),
  );
  expect(
    services.listModelEndpointCapabilitiesService,
...
test("loads models with capability chips", async () => {
  const services = renderPage();
  ...
  expect(services.listModelEndpointCapabilitiesService).toHaveBeenCalledWith({
    pageSize: 500,
    operation: "",
    includeDisabled: false,
  });
...
test("creates credentials from pasted API keys", async () => {
  ...
  renderAccessPage({
    listInferenceCredentialsService: jest
      .fn()
      .mockResolvedValue({ credentials: [], nextPageToken: "" }),
  ...
  await waitFor(() =>
    expect(createInferenceCredentialService).toHaveBeenCalledWith(
...
test("revokes credentials from the credentials table", async () => {
  ...
  expect(
    await screen.findByRole("heading", { name: "Revoke wrong-key?" }),
  ).toBeInTheDocument();
  expect(setInferenceCredentialStatusService).not.toHaveBeenCalled();
  ...
  expect(
    await screen.findByText("Credential wrong-key revoked."),
  ).toBeInTheDocument();
...
test("prompts for package import when no credential endpoints are available", async () => {
  renderAccessPage({
    listModelEndpointsService: jest
      .fn()
      .mockResolvedValue({ modelEndpoints: [], nextPageToken: "" }),
    listInferenceCredentialsService: jest
      .fn()
      .mockResolvedValue({ credentials: [], nextPageToken: "" }),
  });
  ...
  expect(
    await screen.findByText(/No enabled endpoints are available/i),
  ).toBeInTheDocument();
...
test("creates grants with dynamic resource selectors", async () => {
  ...
  const services = renderAccessPage({
    ...
    listInferenceCredentialGrantsService: jest
      .fn()
      .mockResolvedValue({ credentialGrants: [], nextPageToken: "" }),
  ...
  await userEvent.click(
    await screen.findByRole("button", { name: /^Create grant$/i }),
  );
  await userEvent.selectOptions(
  ...
  await waitFor(() =>
    expect(services.listDomainsService).toHaveBeenCalledWith({
      spaceId: "sp1",
      pageSize: 100,
      includeSystem: false,
    }),
  ...
  await userEvent.click(
    screen.getByRole("button", { name: /^create grant$/i }),
  );

  await waitFor(() =>
    expect(createInferenceCredentialGrantService).toHaveBeenCalledWith(
      expect.objectContaining({
        spaceId: "sp1",
        credentialId: "cred1",
        modelEndpointId: "ep1",
...
test("filters grants with dynamic scope dropdowns", async () => {
  const services = renderAccessPage({
    ...
    listInferenceCredentialsService: jest
      .fn()
      .mockResolvedValue({ credentials: [], nextPageToken: "" }),
    listInferenceCredentialGrantsService: jest.fn().mockResolvedValue({
      credentialGrants: [
        {
          credentialGrantId: "grant1",
          credentialId: "cred-main",
          scope: { spaceId: "sp1", domainId: "dom1" },
          operations: ["chat"],
          modelEndpointKey: "openai",
          modelKey: "gpt",
          state: "active",
        ...
        {
          credentialGrantId: "grant2",
          credentialId: "cred-archive",
          scope: { spaceId: "sp1", domainId: "dom2" },
          operations: ["chat"],
          modelEndpointKey: "openai",
          modelKey: "gpt",
          state: "active",
  ...
  await userEvent.selectOptions(
    await screen.findByLabelText(/^Filter space$/i),
    "sp1",
  );
  await waitFor(() =>
    expect(services.listInferenceCredentialGrantsService).toHaveBeenCalledWith({
      spaceId: "sp1",
  ...
  await userEvent.selectOptions(
    await screen.findByLabelText(/^Filter domain$/i),
    "dom1",
  );
  await waitFor(() =>
    expect(screen.queryByText("cred-archive")).not.toBeInTheDocument(),
  );
...
test("creates profiles with dynamic scope and operation selectors", async () => {
  ...
  const services = renderAccessPage({
    ...
    listInferenceProfilesService: jest
      .fn()
      .mockResolvedValue({ inferenceProfiles: [], nextPageToken: "" }),
  ...
  await userEvent.click(
    await screen.findByLabelText("Model refs: openai/text-embedding-3-small"),
  );
  ...
  await waitFor(() =>
    expect(createInferenceProfileService).toHaveBeenCalledWith(
      expect.objectContaining({
        spaceId: "sp1",
        operation: "embeddings",
        domainIds: ["dom1"],
...
test("uses multi-select endpoint and model refs for profile creation", async () => {
  const listModelsService = jest.fn().mockResolvedValue({
    models: [
      {
        modelId: "m-chat",
        key: "openai/gpt-5.6-mini",
        kind: "generative",
  ...
  renderAccessPage({
    listSpacesService: jest
      .fn()
      .mockResolvedValue({ spaces: [], nextPageToken: "" }),
    listInferenceProfilesService: jest
      .fn()
      .mockResolvedValue({ inferenceProfiles: [], nextPageToken: "" }),
    listModelsService,
    listModelEndpointCapabilitiesService: jest.fn().mockResolvedValue({
      modelEndpointCapabilities: [
        {
          modelEndpointCapabilityId: "cap-chat",
          modelEndpointId: "ep1",
          modelEndpointKey: "openai",
          modelId: "m-chat",
          modelKey: "openai/gpt-5.6-mini",
          operation: "chat",
          enabled: true,
        },
        {
          modelEndpointCapabilityId: "cap-embed",
          modelEndpointId: "ep1",
          modelEndpointKey: "openai",
          modelId: "m-embed",
          modelKey: "openai/text-embedding-3-small",
  ...
  await userEvent.click(screen.getByRole("tab", { name: "Profiles" }));
  await waitFor(() =>
    expect(listModelsService).toHaveBeenCalledWith({ pageSize: 500 }),
  );
  await userEvent.selectOptions(
    await screen.findByLabelText(/^Operation$/i),
    "embeddings",
  );

  expect(screen.queryByText("openai/gpt-5.6-mini")).not.toBeInTheDocument();
  expect(
    await screen.findByText("openai/text-embedding-3-small"),
  ).toBeInTheDocument();
...
test("filters policies with dynamic scope dropdowns", async () => {
  const services = renderAccessPage({
    ...
    listInferencePoliciesService: jest.fn().mockResolvedValue({
      inferencePolicies: [
        {
          inferencePolicyId: "policy1",
          scope: { spaceId: "sp1", domainId: "dom1" },
          effect: "allow",
          operations: ["chat"],
          action: "allow",
        ...
        {
          inferencePolicyId: "policy2",
          scope: { spaceId: "sp1", domainId: "dom2" },
          effect: "deny",
          operations: ["chat"],
          action: "deny",
          state: "active",
          reason: "archive policy",
        },
      ],
      nextPageToken: "",
    }),
  ...
  await waitFor(() =>
    expect(services.listInferencePoliciesService).toHaveBeenCalledWith({
      spaceId: "sp1",
      includeExpired: false,
      pageSize: 100,
    }),
  ...
  await waitFor(() =>
    expect(screen.queryByText("archive policy")).not.toBeInTheDocument(),
  );
...
test("creates policies with dynamic scope selectors", async () => {
  ...
  const services = renderAccessPage({
    ...
    listInferencePoliciesService: jest
      .fn()
      .mockResolvedValue({ inferencePolicies: [], nextPageToken: "" }),
  ...
  const createPolicyButtons = screen.getAllByRole("button", {
    name: "Create policy",
  });
  await userEvent.click(createPolicyButtons[createPolicyButtons.length - 1]);

  await waitFor(() =>
    expect(createInferencePolicyService).toHaveBeenCalledWith(
      expect.objectContaining({
        spaceId: "sp1",
        effect: "allow",
...
test("renders empty package state", async () => {
  renderPage({
    listInferencePackagesService: jest
      .fn()
      .mockResolvedValue({ packages: [], nextPageToken: "" }),
  });
  ...
  expect(
    await screen.findByText(/no inference packages imported yet/i),
  ).toBeInTheDocument();
});
...
test("loads additional package pages", async () => {
  const listInferencePackagesService = jest
    ...
    .mockResolvedValueOnce({
      packages: [packagesResponse.packages[0]],
      nextPageToken: "next",
    })
    .mockResolvedValueOnce({
      packages: [
        {
          ...packagesResponse.packages[0],
          inferencePackageId: "pkg-2",
          name: "standard-local",
        },
      ],
      nextPageToken: "",
  ...
  expect(listInferencePackagesService).toHaveBeenLastCalledWith({
    pageSize: 50,
    pageToken: "next",
  });
});
...
test("refresh invokes active catalog service", async () => {
  ...
  await waitFor(() =>
    expect(services.listModelEndpointsService).toHaveBeenCalledTimes(2),
  );
});
...
test("keeps inference catalog readable while hiding package import without manage capability", async () => {
  renderPage({
    principalContext: {
      ...
      capabilityState: {
        kind: "complete",
        capabilities: [{ capability: "CAPABILITY_INFERENCE_CATALOG_READ" }],
      },
      warnings: [],
    },
  });
  ...
  expect(await screen.findByText("Openai Compatible")).toBeInTheDocument();
  expect(
    screen.queryByRole("button", { name: /import package json/i }),
  ).not.toBeInTheDocument();
...
test("imports package JSON and shows summary", async () => {
  ...
  await screen.findByText("Openai Compatible");
  await userEvent.click(
    screen.getByRole("button", { name: /import package json/i }),
  );
  ...
  await waitFor(() =>
    expect(services.applyInferencePackageService).toHaveBeenCalledWith(
      expect.objectContaining({
        name: "standard-openai-chat",
        version: "2026-06",
      }),
  ...
  expect(
    await screen.findByText(/inference package imported/i),
  ).toBeInTheDocument();
  expect(screen.getByText("Model endpoints")).toBeInTheDocument();

  await userEvent.click(
    screen.getByRole("button", { name: /view endpoints/i }),
  );
...
test("renders invalid JSON errors", async () => {
  ...
  await userEvent.type(
    screen.getByLabelText(/or paste package json/i),
    "not json",
  );
  await userEvent.click(
    screen.getByRole("button", { name: /^import package$/i }),
