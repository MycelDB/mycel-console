...
type ProcedureRow = {
  space: SpaceInfo;
  domain: DomainInfo;
  procedure: GraphProcedureSummaryInfo;
};
type BindingRow = {
  space: SpaceInfo;
  domain: DomainInfo;
  binding: GraphAutomationBindingSummaryInfo;
};
type RunRow = {
  space: SpaceInfo;
  domain: DomainInfo;
  automation: AutomationDefinitionSummaryInfo;
  invocation: AutomationInvocationSummaryInfo;
};
...
export function AutomationsPage({
  ...
  const [detail, setDetail] = useState<{ title: string; json: string } | null>(
    null,
  );
  ...
  const load = useCallback(async () => {
    ...
    setUsageError("");
    try {
      const principalListPromise = canReadPrincipals
        ? listPrincipalsService({
            pageSize: 100,
      ...
      setPrincipals((await principalListPromise)?.principals ?? []);

      const domainResults = await Promise.all(
        spaceResponse.spaces.map(async (space) => {
      ...
      const spaceById = new Map(
        spaceResponse.spaces.map((space) => [space.spaceId, space]),
      );
      const domainCandidates = allDomains.filter(
        (domain) =>
          (!selectedSpaceId || domain.spaceId === selectedSpaceId) &&
          (!selectedDomainId || domain.domainId === selectedDomainId),
      );
      const automationRows = await Promise.all(
        domainCandidates.map(async (domain) => {
          const automations = await listAutomationsService({
            domainId: domain.domainId,
          });
          const invocationPairs = await Promise.all(
            automations.automations.map(async (automation) => {
              try {
          ...
          return automations.automations.map((automation) => ({
            space: spaceById.get(domain.spaceId) ?? {
      ...
      const procedureAndBindingRows = await Promise.all(
        domainCandidates.map(async (domain) => {
          const space = spaceById.get(domain.spaceId) ?? {
            spaceId: domain.spaceId,
            name: domain.spaceId,
          };
          const [procedures, bindings] = await Promise.all([
            listGraphProceduresService({ domainId: domain.domainId }).catch(
              () => ({ procedures: [] as GraphProcedureSummaryInfo[] }),
            ),
            listGraphAutomationBindingsService({
              domainId: domain.domainId,
            }).catch(() => ({
              bindings: [] as GraphAutomationBindingSummaryInfo[],
            })),
          ]);
          return {
            procedures: procedures.procedures.map((procedure) => ({
              space,
              domain,
              procedure,
            })),
      ...
      await Promise.all(
        usageSpaces.map(async (space) => {
          try {
            ...
            setUsageError(
              errorMessage(err, "Failed to load automation usage summaries"),
            );
          }
        }),
      );
  ...
  const filteredRows = useMemo(
    () =>
      rows.filter(
        (row) => !statusFilter || row.automation.status === statusFilter,
      ),
    [rows, statusFilter],
  );
  const filteredProcedureRows = useMemo(
    () =>
      procedureRows.filter(
        (row) => !statusFilter || row.procedure.status === statusFilter,
      ),
    [procedureRows, statusFilter],
  );
  ...
  const filteredBindingRows = useMemo(
    () =>
      bindingRows.filter(
        (row) => !statusFilter || row.binding.status === statusFilter,
      ),
    [bindingRows, statusFilter],
  );
  const runRows = useMemo(
    () =>
      rows.flatMap((row) =>
        row.invocations.map((invocation) => ({
          space: row.space,
          domain: row.domain,
  ...
  const filteredRunRows = useMemo(
    () =>
      runRows.filter(
        (row) => !statusFilter || row.invocation.status === statusFilter,
      ),
    [runRows, statusFilter],
  );
  const statusOptions =
    activeTab === "runs"
      ? [
  ...
  const totals = useMemo(() => {
    const usage = rows.reduce(
      (sum, row) =>
        sum +
        (usageByAutomation[
          automationUsageKey(row.domain.domainId, row.automation.id)
        ]?.totalTokens ?? 0),
      0,
    );
    return {
    ...
    filteredBindingRows.length,
    filteredProcedureRows.length,
    filteredRows.length,
  ...
  async function showProcedure(domainId: string, procedureId: string) {
    setError("");
    try {
      const response = await getGraphProcedureService({
        domainId,
        procedureId,
      });
      setDetail({
        title: `Procedure ${procedureId}`,
        json: response.procedureJson,
      });
    } catch (err) {
  ...
  async function showBinding(domainId: string, bindingId: string) {
    setError("");
    try {
      const response = await getGraphAutomationBindingService({
        domainId,
        bindingId,
      });
  ...
  async function openCreate(
    domainId = selectedDomainId ||
      filteredDomains[0]?.domainId ||
      domains[0]?.domainId ||
      "",
  ) {
    const domain = domains.find((candidate) => candidate.domainId === domainId);
    const spaceId =
      domain?.spaceId || selectedSpaceId || spaces[0]?.spaceId || "";
  ...
  async function openEdit(row: AutomationRow) {
    ...
    try {
      ...
      let selectedProfile = "";
      try {
        const parsed = JSON.parse(detailResponse.definitionJson) as {
      ...
      setEditor({
        ...
        domainId: row.domain.domainId,
        automationId: row.automation.id,
        definitionJson: detailResponse.definitionJson,
        selectedProfile,
        profiles,
      });
      setEditorError("");
  ...
  async function saveEditor() {
    ...
    try {
      ...
      const definitionJson =
        validation.normalizedDefinitionJson || editor.definitionJson;
  ...
  function applyProfile(profileKeyOrId: string) {
    setEditor((current) => {
      ...
      try {
        ...
        return {
          ...current,
          selectedProfile: profileKeyOrId,
  ...
  async function toggleAutomation(row: AutomationRow) {
    ...
    try {
      ...
      if (row.automation.status === "enabled")
        await disableAutomationService(input);
  ...
  async function toggleBinding(row: BindingRow) {
    ...
    try {
      ...
      if (row.binding.status === "enabled")
        await disableGraphAutomationBindingService(input);
  ...
  return (
    <section className="space-y-6">
      ...
      <div
        className={`grid gap-4 rounded-xl border ${themeClasses.border.default} ${themeClasses.surface.panel} p-4 md:grid-cols-3`}
      >
        <Select
          label="Space"
      ...
      >
        <Tabs
          ariaLabel="Graph automation sections"
          className="p-3"
          tabs={[
          ...
          <div>
            <div className="border-b border-slate-200 px-4 py-3 dark:border-slate-800">
              ...
              <Text intent="muted" size="sm">
                Reusable graph work: context assembly, inference defaults,
                output actions, and safety ceilings.
              </Text>
            </div>
            <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
              ...
              >
                <tr>
                  ...
                  <TableHead className="px-4 py-3">Inference</TableHead>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                ...
                ) : filteredProcedureRows.length === 0 ? (
                  <tr>
                    <td
                      className={`px-4 py-6 text-center ${themeClasses.text.parts.subtleLight} ${themeClasses.text.parts.darkMuted}`}
                      colSpan={5}
                    >
                      No graph procedures found.
                    </td>
                  </tr>
                ) : (
                  filteredProcedureRows.map((row) => (
                    <tr key={`${row.domain.domainId}:${row.procedure.id}`}>
                      <td className="px-4 py-3">
                        <div
                          className={`font-medium ${themeClasses.text.parts.primaryLight} ${themeClasses.text.parts.darkPrimary}`}
                        >
                          {row.procedure.name || row.procedure.id}
                        </div>
                        <div
                          className={`mt-1 text-xs ${themeClasses.text.parts.mutedLight} ${themeClasses.text.parts.darkMuted}`}
                        >
                          <ResourceIdText value={row.procedure.id} /> · v
                          {row.procedure.version}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <SpaceLabel
                      ...
                      <td className="px-4 py-3">
                        <StatusPill value={row.procedure.status} />
                      </td>
                      <td className="px-4 py-3">
                        <div>{formatEnumLabel(row.procedure.operation)}</div>
                        <div
                          className={`text-xs ${themeClasses.text.parts.mutedLight}`}
                        >
                          {row.procedure.inferenceProfile ||
                            "No default profile"}
                          {row.procedure.inferenceProfileId &&
                          row.procedure.inferenceProfile !==
                            row.procedure.inferenceProfileId ? (
                            <>
                              {" "}
                              (
                              <ResourceIdText
                                value={row.procedure.inferenceProfileId}
                              />
                              )
                            </>
                          ) : null}
        ...
        {activeTab === "automations" && (
          <div>
            <div className="border-b border-slate-200 px-4 py-3 dark:border-slate-800">
              ...
              <Text intent="muted" size="sm">
                Bindings that connect procedures to triggers, scopes, runtime
                principals, and inference profiles.
              </Text>
            </div>
            {filteredBindingRows.length > 0 ? (
              <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
                ...
                >
                  <tr>
                    ...
                    <TableHead className="px-4 py-3">Scope</TableHead>
                    <TableHead className="px-4 py-3">Runtime</TableHead>
                    <TableHead className="px-4 py-3">Trigger</TableHead>
                    <TableHead className="px-4 py-3">Actions</TableHead>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                  {filteredBindingRows.map((row) => (
                    <tr key={`${row.domain.domainId}:${row.binding.id}`}>
                      <td className="px-4 py-3">
                        <div
                          className={`font-medium ${themeClasses.text.parts.primaryLight} ${themeClasses.text.parts.darkPrimary}`}
                        >
                          {row.binding.name || row.binding.id}
                        </div>
                        <div
                          className={`mt-1 text-xs ${themeClasses.text.parts.mutedLight} ${themeClasses.text.parts.darkMuted}`}
                        >
                          <ResourceIdText value={row.binding.id} /> ·{" "}
                          <StatusPill value={row.binding.status} />
                        </div>
                        <div
                          className={`mt-1 text-xs ${themeClasses.text.parts.mutedLight}`}
                        >
                          Procedure{" "}
                          <ResourceIdText value={row.binding.procedureId} /> v
                          {row.binding.procedureVersion}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <SpaceLabel
                      ...
                      <td className="px-4 py-3">
                        <div className="text-xs">
                          actor{" "}
                          <RuntimePrincipalLabel
                            principalId={row.binding.actorPrincipalId}
                        ...
                        <div className="text-xs">
                          on behalf{" "}
                          <RuntimePrincipalLabel
                            principalId={row.binding.onBehalfOfPrincipalId}
                            principalContext={principalContext}
                            principalById={principalById}
                          />
                        </div>
                        <div className="text-xs">
                          owner{" "}
                          <RuntimePrincipalLabel
                            principalId={row.binding.ownerPrincipalId}
                            principalContext={principalContext}
                            principalById={principalById}
                          />
                        </div>
                      ...
                      <td className="px-4 py-3">
                        <div>{formatEnumLabel(row.binding.triggerType)}</div>
                        <div
                          className={`text-xs ${themeClasses.text.parts.mutedLight}`}
                        >
                          {row.binding.events.join(", ") || "No events"}
                        </div>
                        <div
                          className={`text-xs ${themeClasses.text.parts.mutedLight}`}
                        >
                          {row.binding.labels.join(", ") || "No labels"}
                        </div>
                      </td>
                      <td className="px-4 py-3">
              ...
              <LegacyAutomationTable
                rows={filteredRows}
                loading={loading}
                usageByAutomation={usageByAutomation}
                canManage={canManage}
                actionLoading={actionLoading}
                onShowAutomation={showAutomation}
                onEdit={openEdit}
        ...
        {activeTab === "runs" && (
          <div>
            <div className="border-b border-slate-200 px-4 py-3 dark:border-slate-800">
              ...
              <Text intent="muted" size="sm">
                Recent invocation/run attempts with status, trigger event,
                target, and run detail links.
              </Text>
            </div>
            <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
              <thead
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {loading ? (
                  ...
                  filteredRunRows.map((row) => (
                    <tr key={`${row.domain.domainId}:${row.invocation.id}`}>
                      <td className="px-4 py-3">
                        <ResourceIdText
                          value={row.invocation.id}
                          className={`${themeClasses.text.parts.primaryLight} ${themeClasses.text.parts.darkPrimary}`}
                        />
                        <div
                          className={`text-xs ${themeClasses.text.parts.mutedLight}`}
                        >
                          {formatTimestamp(
                            row.invocation.updatedAt ||
                              row.invocation.createdAt,
                          )}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <div
                          className={`font-medium ${themeClasses.text.parts.primaryLight} ${themeClasses.text.parts.darkPrimary}`}
                        >
                          {row.automation.name || row.automation.id}
                        </div>
                        <div
                          className={`mt-1 text-xs ${themeClasses.text.parts.mutedLight} ${themeClasses.text.parts.darkMuted}`}
                        >
                          <ResourceIdText value={row.invocation.automationId} />{" "}
                          · v{row.invocation.automationVersion}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <SpaceLabel
                      ...
                      <td className="px-4 py-3">
                        <div>{formatEnumLabel(row.invocation.eventType)}</div>
                        <div
                          className={`text-xs ${themeClasses.text.parts.mutedLight}`}
                        >
                          target{" "}
                          <ResourceIdText
                            value={
                              row.invocation.changedElementId ||
                              row.invocation.eventId
                            }
                          />
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <StatusPill value={row.invocation.status} />
                        {row.invocation.skipReason ? (
                          <div
                            className={`mt-1 text-xs ${themeClasses.text.parts.mutedLight}`}
                          >
                            {row.invocation.skipReason}
                      ...
                      <td className="px-4 py-3">
                        <Button
                          variant="secondary"
                          onClick={() =>
                            void showRun(row.domain.domainId, row.invocation.id)
                          }
                        >
                          View JSON
                        </Button>
      ...
      {editor && (
        <AutomationEditorDialog
          ...
          onDefinitionChange={(definitionJson) =>
            setEditor((current) =>
          ...
          onDomainChange={async (domainId) => {
            ...
            setEditor((current) =>
              current
                ? {
                    ...current,
                    spaceId: domain?.spaceId || current.spaceId,
                    domainId,
                    profiles,
                  }
                : current,
...
function compareAutomationRows(a: AutomationRow, b: AutomationRow) {
  return `${a.space.name || a.space.spaceId}:${a.domain.name || a.domain.key}:${a.automation.name || a.automation.id}`.localeCompare(
    `${b.space.name || b.space.spaceId}:${b.domain.name || b.domain.key}:${b.automation.name || b.automation.id}`,
  );
}
...
function LegacyAutomationTable({
  rows,
  loading,
  usageByAutomation,
  canManage,
  ...
  onEdit: (row: AutomationRow) => void | Promise<void>;
  onToggle: (row: AutomationRow) => void | Promise<void>;
  onDelete: (row: AutomationRow) => void | Promise<void>;
}) {
  return (
    <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
      <thead
      ...
      <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
        {loading ? (
          ...
          rows.map((row) => {
            ...
            return (
              <tr
                key={`${row.domain.domainId}:${row.automation.id}`}
              ...
              >
                ...
                <td className="px-4 py-3">
                  ...
                  <div
                    className={`mt-1 text-xs ${themeClasses.text.parts.mutedLight} ${themeClasses.text.parts.darkMuted}`}
                  >
                    <ResourceIdText value={row.automation.id} /> · v
                    {row.automation.version}
                  </div>
                  <div
                    className={`mt-2 text-xs ${themeClasses.text.parts.subtleLight} ${themeClasses.text.parts.darkMuted}`}
                  >
                    {row.automation.events.join(", ") || "No event filter"}
                  </div>
                </td>
                <td className="px-4 py-3">
                  <StatusPill value={row.automation.status} />
                </td>
                <td className="px-4 py-3">
                  {usage ? (
                    <div>
...
function RuntimePrincipalLabel({
  principalId,
  principalContext,
  principalById,
  empty = "—",
  ...
  const principal = principalById?.get(id);
  const session = principalContext?.session;
  const display =
    principal?.displayName ||
    principal?.username ||
    (session?.principalId === id ? session.username : "") ||
    shortResourceId(id);
  return (
    <TextLink to={`/principals/${encodeURIComponent(id)}`} title={id}>
      {display}
    </TextLink>
  );
}
...
function RecentInvocations({
  ...
  domainId: string;
  invocations: AutomationInvocationSummaryInfo[];
  onShowRun: (domainId: string, runId: string) => void | Promise<void>;
}) {
    ...
    <div className="space-y-2">
      {invocations.map((invocation) => (
        <button
          key={invocation.id}
          type="button"
          className="block text-left text-sky-700 hover:text-sky-900 dark:text-sky-300 dark:hover:text-sky-100"
          onClick={() => void onShowRun(domainId, invocation.id)}
        >
          <ResourceIdText value={invocation.id} />
          <span
            className={`ml-2 text-xs ${themeClasses.text.parts.mutedLight}`}
          >
            {formatEnumLabel(invocation.status)}
          </span>
        </button>
      ))}
    </div>
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <Text intent="muted" size="sm" className="mt-1">
              Raw automation diagnostic payload.
            </Text>
          </div>
...
function AutomationEditorDialog({
  ...
  onClose: () => void;
  onSave: () => void;
  onDefinitionChange: (definitionJson: string) => void;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4">
      <div
        className={`max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-xl border ${themeClasses.border.default} ${themeClasses.surface.elevated} p-6 shadow-xl`}
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <Text
            ...
            <Text intent="muted" size="sm" className="mt-1">
              Definitions reference inference profiles and model configuration
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <Select
            ...
            value={editor.selectedProfile}
            onChange={onProfileChange}
            options={editor.profiles.map((profile) => ({
              value: profile.key || profile.inferenceProfileId,
              label: `${profile.displayName || profile.key} (${formatEnumLabel(profile.operation)})`,
            }))}
        ...
        <label
          className={`mt-4 block text-sm font-medium ${themeClasses.text.parts.bodyLight} ${themeClasses.text.parts.darkStrong}`}
          htmlFor="automation-definition-json"
        >
          Definition JSON
        </label>
        <textarea
          id="automation-definition-json"
          className={`mt-2 min-h-[28rem] w-full rounded-lg border border-slate-300 ${themeClasses.surface.input} p-3 font-mono text-sm ${themeClasses.text.parts.primaryLight} dark:border-slate-700 ${themeClasses.text.parts.darkPrimary}`}
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <Button onClick={onSave} disabled={loading || !editor.domainId}>
