...
function renderPage(
  overrides: Partial<Parameters<typeof SemanticPage>[0]> = {},
) {
  const services = {
    ...
    listInferenceProfilesService: jest.fn().mockResolvedValue({
      inferenceProfiles: [
        {
    ...
    listSemanticRulesService: jest.fn().mockResolvedValue({
      rules: [
        {
          semanticRuleId: "rule1",
          key: "page-summary",
          displayName: "Page summary",
          description: "Summaries for pages",
          spaceId: "sp1",
          domainId: "dom1",
          enabled: true,
          ...
          bindings: [
            {
              ...
              vectorStoreId: "",
              vectorStoreKey: "local",
              enabled: true,
              searchIndex: {
                state: "SEARCH_INDEX_STATE_READY",
                liveRecordCount: 3,
                lastRebuildAt: "",
                lastError: "",
              },
    ...
    getSemanticRuleService: jest
      .fn()
      .mockResolvedValue({ rule: null, summary: null }),
    validateSemanticRuleService: jest
      .fn()
    ...
    summarizeInferenceUsageService: jest.fn().mockResolvedValue({
      summaries: [
        {
          group: { semantic_rule_id: "rule1", domain_id: "dom1" },
          requestCount: 3,
          succeededCount: 3,
          failedCount: 0,
          deniedCount: 0,
    ...
    semanticSearchService: jest.fn().mockResolvedValue({
      results: [
        {
          semanticRuleId: "rule1",
          embeddingBindingKey: "search",
          ...
          node: {
            nodeId: "node1",
            properties: { title: "Semantic troubleshooting" },
          },
          matchedChunkIds: [],
          snippet: "stale search indexes",
        },
      ],
      warnings: [],
    }),
  ...
  render(
    <MemoryRouter>
      <SemanticPage
        {...services}
        principalContext={{
          session: { addr: "", principalId: "alice", username: "alice" },
          roles: ["semantic.admin"],
          capabilities: ["semantic.manage"],
          capabilityState: {
            kind: "complete",
            capabilities: [{ capability: "semantic.manage" }],
          },
          warnings: [],
        }}
      />
...
test("renders global semantic rule inventory, maintenance, and usage", async () => {
  ...
  expect(listInferenceProfilesService).toHaveBeenCalledWith({
    spaceId: "sp1",
    domainId: undefined,
    operation: "embeddings",
...
test("shows embedding generation activity", async () => {
  ...
  expect(
    screen.getByRole("heading", { name: /embedding generation activity/i }),
  ).toBeInTheDocument();
...
test("uses an Intelligence profile dropdown in the rule editor", async () => {
  ...
  expect(screen.getByLabelText(/intelligence profile/i)).toHaveDisplayValue(
    "Select profile",
  );
  await userEvent.selectOptions(
    screen.getByLabelText(/intelligence profile/i),
    "embedding-default",
  );
  expect(screen.getByLabelText(/intelligence profile/i)).toHaveValue(
    "embedding-default",
  );
});

test("omits GQL-only selector fields for node type semantic rules", async () => {
  const validateSemanticRuleService = jest
    .fn()
    .mockResolvedValue({ valid: true, diagnostics: [] });
  ...
  expect(validateSemanticRuleService).toHaveBeenCalledWith(
...
test("defaults target alias only for GQL selectors", async () => {
  ...
  await userEvent.selectOptions(
    screen.getByLabelText(/^selector$/i),
    "node_type",
  );
  expect(screen.queryByLabelText(/target alias/i)).not.toBeInTheDocument();
});
...
test("runs semantic search from the search panel", async () => {
  ...
  await userEvent.selectOptions(
    screen.getByLabelText(/search domain/i),
    "dom1",
  );
  ...
  expect(
    await screen.findByText("Semantic troubleshooting"),
  ).toBeInTheDocument();
  expect(semanticSearchService).toHaveBeenCalledWith(
    expect.objectContaining({
      spaceId: "sp1",
      domainId: "dom1",
      query: "stale indexes",
