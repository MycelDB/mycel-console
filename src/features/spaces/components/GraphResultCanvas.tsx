...
export function GraphResultCanvas({
  graph,
  className = "",
}: GraphResultCanvasProps) {
    ...
    void import("cytoscape").then(({ default: cytoscape }) => {
      ...
      const selectHandler = (event: EventObject) => {
        ...
        setSelected({
          ...
          labels: Array.isArray(element.data("labels"))
            ? element.data("labels")
            : [],
  ...
  if (mapping.elements.length === 0) {
    return (
      <div
        className={`mt-3 rounded-lg border border-dashed border-slate-300 p-4 text-sm ${themeClasses.text.parts.subtleLight} dark:border-slate-700 ${themeClasses.text.parts.darkMuted} ${className}`}
      >
        No graph elements returned. Return node or relationship values to
  ...
  return (
    <div
      className={`mt-3 rounded-lg border ${themeClasses.border.default} ${themeClasses.surface.panel} p-4 ${className}`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="secondary" onClick={fitGraph}>
            Fit
          </Button>
          <Button variant="secondary" onClick={relayoutGraph}>
            Relayout
          </Button>
          <Button
            variant="secondary"
...
function GraphWarnings({
  ...
  mapping: ReturnType<typeof graphResultToCytoscapeElements>;
  elementCount: number;
}) {
  const warnings: string[] = [];
  if (elementCount > LARGE_GRAPH_THRESHOLD)
    warnings.push(
      `Rendering ${elementCount} graph elements. Large result sets may be slower to pan, zoom, or lay out.`,
    );
  if (mapping.omittedEdgeCount > 0)
    warnings.push(
      `${mapping.omittedEdgeCount} ${mapping.omittedEdgeCount === 1 ? "edge was" : "edges were"} omitted because endpoints were not returned.`,
    );
  if (mapping.duplicateNodeCount > 0 || mapping.duplicateEdgeCount > 0)
    warnings.push(
      "Duplicate graph elements were collapsed in the visualization.",
    );
  ...
  return (
    <div className="mt-3 space-y-2">
      {warnings.map((warning) => (
        <div
          key={warning}
          className="rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-200"
        >
          {warning}
        </div>
      ))}
...
function GraphSelectionInspector({
  ...
  selected: SelectedGraphElement | null;
}) {
  return (
    <aside className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/40">
      ...
      {!selected ? (
        ...
        <div className="mt-3 space-y-3 text-sm">
          ...
          <div>
            <span className="font-medium">Label:</span> {selected.label}
          </div>
          <div>
            <span className="font-medium">ID:</span>{" "}
            <ResourceIdText value={selected.id} />
          </div>
          <div>
            <span className="font-medium">Labels:</span>{" "}
            {selected.labels.length ? selected.labels.join(", ") : "—"}
          </div>
          <pre
            className={`max-h-96 overflow-auto rounded-md border ${themeClasses.border.default} ${themeClasses.surface.panel} p-3 text-xs ${themeClasses.text.parts.bodyLight} dark:border-slate-800 ${themeClasses.text.parts.darkSecondary}`}
          >
