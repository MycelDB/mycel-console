    ...
    <div className="overflow-hidden rounded-xl border border-slate-800 bg-slate-900/70">
      <table className="min-w-full divide-y divide-slate-800">
        ...
        <tbody className="divide-y divide-slate-800">
          {spaces.map((space) => (
            <tr key={space.spaceId} className="hover:bg-slate-800/40">
              <td className="px-4 py-3 font-medium text-slate-100">{space.name}</td>
              <td className="px-4 py-3 font-mono text-xs text-slate-400">{space.spaceId}</td>
