...
import {
  ...
  listPrincipalSessions as defaultListPrincipalSessions,
  listSpaces as defaultListSpaces,
  revokePrincipalCapability as defaultRevokePrincipalCapability,
  revokePrincipalRole as defaultRevokePrincipalRole,
  revokePrincipalSession as defaultRevokePrincipalSession,
  revokePrincipalSessions as defaultRevokePrincipalSessions,
  setPrincipalCapabilitiesForScope as defaultSetPrincipalCapabilitiesForScope,
  setPrincipalRolesForScope as defaultSetPrincipalRolesForScope,
...
import { SpaceStateBadge } from "../../spaces/components/SpaceStateBadge";
...
export type UserDetailPageProps = {
  ...
  revokePrincipalSessionService?: (
    input: RevokePrincipalSessionInput,
  ) => Promise<void>;
  revokePrincipalSessionsService?: (
    principalId: string,
...
type RevokeDialog =
  | { kind: "role"; grantId: string; label: string; reason: string }
  | { kind: "capability"; grantId: string; label: string; reason: string }
...
const CAPABILITY_OPTIONS = [
  ...
  "CAPABILITY_SPACE_ARCHIVE",
  "CAPABILITY_SPACE_DELETE",
  "CAPABILITY_SPACE_CREATE",
  ...
  "CAPABILITY_INFERENCE_CATALOG_MANAGE",
  "CAPABILITY_INFERENCE_PROFILE_READ",
  "CAPABILITY_INFERENCE_PROFILE_MANAGE",
  "CAPABILITY_INFERENCE_CREDENTIAL_READ",
  "CAPABILITY_INFERENCE_CREDENTIAL_MANAGE",
  "CAPABILITY_INFERENCE_GRANT_MANAGE",
  "CAPABILITY_INFERENCE_POLICY_MANAGE",
  "CAPABILITY_INFERENCE_AUDIT_READ",
  "CAPABILITY_INFERENCE_INVOKE",
  "CAPABILITY_AUTOMATION_READ",
  "CAPABILITY_AUTOMATION_MANAGE",
  "CAPABILITY_AUTOMATION_RUN",
  "CAPABILITY_AUTOMATION_WORKER",
...
export function UserDetailPage({
  ...
  const [roles, setRoles] = useState<ListPrincipalRolesResponse>({
    grants: [],
    effectiveRoles: [],
  });
  const [capabilities, setCapabilities] =
    useState<ListPrincipalCapabilitiesResponse>({
      grants: [],
  ...
  const canManageSessions = canUseCapability(
    principalContext,
    "identity.session.manage",
  );
  const canManageGrants = canUseCapability(
    principalContext,
    "identity.grant.manage",
  );
  ...
  async function load() {
    ...
    try {
      ...
      setOwnedSpaces(
        spacesResponse.spaces.filter(
          (space) => space.owner?.id === routePrincipalId,
        ),
      );
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (tab === "access" || tab === "sessions" || tab === "overview")
      setActiveTab(tab);
  }, [searchParams]);
  ...
  async function confirmRevoke() {
    ...
    try {
      if (revokeDialog.kind === "session") {
        await revokePrincipalSessionService({
          principalId: routePrincipalId,
          authSessionId: revokeDialog.sessionId,
        });
        ...
        setMessage(
          `${response.revokedCount} session${response.revokedCount === 1 ? "" : "s"} revoked.`,
        );
        await load();
      } else if (revokeDialog.kind === "role") {
        await revokePrincipalRoleService({
          principalId: routePrincipalId,
          roleGrantId: revokeDialog.grantId,
          reason: revokeDialog.reason,
        });
        setMessage("Role grant revoked.");
        await reloadAccess();
      } else {
        await revokePrincipalCapabilityService({
          principalId: routePrincipalId,
          capabilityGrantId: revokeDialog.grantId,
          reason: revokeDialog.reason,
        });
  ...
  const directGrantCount =
    (roles.grants.length || 0) + (capabilities.grants.length || 0);
  ...
  return (
    <section className="space-y-6">
      ...
      <Tabs<DetailTab>
        ariaLabel="Principal detail sections"
        tabs={[
          { id: "overview", label: "Overview" },
          { id: "access", label: "Roles & capabilities" },
      ...
      {loading ? (
        ...
        <>
          ...
          <OwnedSpaces spaces={ownedSpaces} />
        </>
      ) : null}
      {!loading && activeTab === "access" && (
        <AccessPanel
          ...
          onRefresh={() => void reloadAccess()}
          onSaveRoles={async (input) => {
            await setPrincipalRolesForScopeService(input);
          }}
          onSaveCapabilities={async (input) => {
            await setPrincipalCapabilitiesForScopeService(input);
            await reloadAccess();
          }}
          onRevokeRole={(grant) =>
            setRevokeDialog({
              kind: "role",
              grantId: grant.roleGrantId,
              label: grant.role,
              reason: "",
            })
          }
          onRevokeCapability={(grant) =>
            setRevokeDialog({
              kind: "capability",
              grantId: grant.capabilityGrantId,
      ...
      {!loading && activeTab === "sessions" && (
        <UserSessionsTable
          sessions={sessions}
          canRevoke={canManageSessions}
          onRevokeSession={(sessionId) =>
            setRevokeDialog({ kind: "session", sessionId })
          }
          onRevokeAll={() => setRevokeDialog({ kind: "allSessions" })}
        />
      )}
      {revokeDialog && (
        <ConfirmRevokeDialog
          dialog={revokeDialog}
          setDialog={setRevokeDialog}
...
function Loading() {
  return (
    ...
    >
      <Text intent="muted">Loading principal…</Text>
    </div>
  );
}
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Panel title="Identity">
        <DetailRow label="Principal">
          <PrincipalLabel
            principalId={principalIdOf(user)}
        ...
        <DetailRow
          label="Type"
          value={formatEnumLabel(user.type, "Not reported")}
        />
...
function AccessPanel({
  ...
  capabilities: ListPrincipalCapabilitiesResponse;
  directGrantCount: number;
  canManage: boolean;
  spaces: SpaceInfo[];
  listDomainsService: (input: ListDomainsInput) => Promise<ListDomainsResponse>;
  onRefresh: () => void;
  onSaveRoles: (input: SetPrincipalRolesForScopeInput) => Promise<void>;
  onSaveCapabilities: (
    input: SetPrincipalCapabilitiesForScopeInput,
  ) => Promise<void>;
  onRevokeRole: (grant: PrincipalRoleGrantInfo) => void;
  onRevokeCapability: (grant: PrincipalCapabilityGrantInfo) => void;
}) {
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <Text intent="muted" size="sm" className="mt-1">
            Choose a scope, check the direct roles and extra direct
            capabilities, then Save. Role-inherited capabilities are shown
            checked and locked.
          </Text>
        </div>
      ...
      {!canManage && (
        <Alert>
          Current principal is missing identity.grant.manage; access grants are
          read-only.
        </Alert>
      )}
      <div className="max-w-xs">
        <SummaryCard label="Direct grants" value={directGrantCount} />
      </div>
      {principal && (
        <AccessCheckboxEditor
      ...
      <CapabilityGrantTable
        ...
        canManage={canManage}
        onRevoke={onRevokeCapability}
      />
      {principal && (
        <Text intent="muted" size="sm">
          Editing access for {principal.username || principalIdOf(principal)}.
        </Text>
      )}
    </div>
  );
...
function AccessCheckboxEditor({
  ...
  useEffect(() => {
    if (!form.spaceId || form.scopeType === "system") {
      setDomains([]);
      return;
    }
    let cancelled = false;
    void listDomainsService({
      spaceId: form.spaceId,
      pageSize: 100,
      includeSystem: false,
    })
      .then((response) => {
        if (!cancelled) setDomains(response.domains);
      })
      .catch(() => {
  ...
  const disabled =
    !canManage ||
    saving ||
    (form.scopeType !== "system" && !form.spaceId) ||
    (form.scopeType === "domain" && !form.domainId);
  ...
  return (
    ...
    >
      ...
      {saveMessage && (
        <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950 dark:text-emerald-100">
          {saveMessage}
        </div>
      )}
      <div className="mt-4">
      ...
      <label
        className={`mt-4 block text-sm font-medium ${themeClasses.text.parts.primaryLight} ${themeClasses.text.parts.darkPrimary}`}
      >
        Set reason
        <textarea
          className={`mt-1 block min-h-16 w-full rounded-md border border-slate-300 ${themeClasses.surface.input} px-3 py-2 text-sm ${themeClasses.text.parts.primaryLight} ${themeClasses.focus.ring} dark:border-slate-700 ${themeClasses.text.parts.darkPrimary}`}
          value={form.reason}
          onChange={(event) =>
            setForm((current) => ({ ...current, reason: event.target.value }))
          }
          placeholder="Audit reason for set operation"
        />
      </label>
      <div className="mt-5 grid gap-6 lg:grid-cols-2">
        <CheckboxGroup
          ...
          disabled={disabled}
          onToggle={(role) =>
            setSelectedRoles((current) => toggleValue(current, role))
          }
        />
        <CheckboxGroup
          title="Direct capabilities"
          ...
          onToggle={(capability) =>
            setSelectedCapabilities((current) =>
              toggleValue(current, capability),
            )
          }
        />
      </div>
...
function CheckboxGroup({
  ...
  return (
    <div>
      ...
      <div className="mt-3 max-h-80 space-y-2 overflow-auto rounded-lg border border-slate-200 p-3 dark:border-slate-800">
        {options.map((value) => {
          const inheritedOnly =
            inherited.has(value) && !selected.includes(value);
          const checked = selected.includes(value) || inherited.has(value);
          return (
            <label
              key={value}
              className={`flex items-center justify-between gap-3 rounded px-2 py-1 text-sm ${inheritedOnly ? `${themeClasses.text.parts.mutedLight} ${themeClasses.text.parts.darkMuted}` : `${themeClasses.text.parts.strongLight} ${themeClasses.text.parts.darkStrong}`}`}
            >
              <span className="font-mono text-xs">{value}</span>
              <span className="flex items-center gap-2">
                <input
                  aria-label={inheritedOnly ? `${value} inherited` : value}
                  type="checkbox"
                  checked={checked}
                  disabled={disabled || inheritedOnly}
                  onChange={() => onToggle(value)}
                />
                {inheritedOnly && (
                  <span
                    className={`rounded bg-slate-100 px-2 py-0.5 text-[10px] uppercase tracking-wide ${themeClasses.text.parts.subtleLight} dark:bg-slate-800 ${themeClasses.text.parts.darkSecondary}`}
                  >
                    role
                  </span>
                )}
...
function RoleGrantTable({
  ...
  canManage: boolean;
  onRevoke: (grant: PrincipalRoleGrantInfo) => void;
}) {
  return (
    <GrantTable
      title="Role grants"
...
function CapabilityGrantTable({
  ...
  canManage: boolean;
  onRevoke: (grant: PrincipalCapabilityGrantInfo) => void;
}) {
  return (
    <GrantTable
      title="Capability grants"
      empty="No direct capability grants"
      headers={["Capability", "Scope", "Reason", "Actions"]}
      rows={grants.map((grant) => [
        grant.capability,
        scopeLabel(grant.scope),
        grant.reason || "—",
...
function ScopeEditor({
  ...
  form: GrantForm;
  setForm: (value: GrantForm | ((current: GrantForm) => GrantForm)) => void;
  spaces: SpaceInfo[];
  domains: DomainInfo[];
  labelPrefix?: string;
}) {
  const prefix = labelPrefix.trim();
  const scopeLabelText = (base: string) =>
    prefix ? `${prefix} ${base.toLowerCase()}` : base;
  return (
    <div className="rounded-lg border border-slate-200 p-4 dark:border-slate-800">
      ...
      <div className="mt-3 flex flex-wrap gap-3">
        {(["system", "space", "domain"] as GrantForm["scopeType"][]).map(
          (scopeType) => (
            <label
              key={scopeType}
              ...
              <input
                ...
                onChange={() =>
                  setForm((current) => ({
                    ...current,
                    scopeType,
                    domainId: scopeType === "domain" ? current.domainId : "",
                  }))
                }
              />
              {scopeType === "system"
                ? scopeLabelText("System-wide")
                : scopeType === "space"
                  ? scopeLabelText("Space")
                  : scopeLabelText("Domain")}
            </label>
          ),
        )}
      </div>
      {form.scopeType !== "system" && (
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          <Select
            label={scopeLabelText("Space")}
            value={form.spaceId}
            onChange={(spaceId) =>
              setForm((current) => ({ ...current, spaceId, domainId: "" }))
            }
      ...
      {form.scopeType === "system" && (
        <Text intent="danger" size="sm" className="mt-3">
          System-wide grants apply across all spaces. Prefer scoped grants when
          possible.
        </Text>
      )}
    </div>
      ...
      {sessions.length === 0 ? (
        ...
        <div className="mt-4 overflow-hidden rounded-lg border border-slate-200 dark:border-slate-800">
          <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
            ...
            >
              <tr>
                <TableHead className="px-4 py-3">Session ID</TableHead>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {sessions.map((session) => (
                <tr key={session.authSessionId}>
                  <td className="px-4 py-3">
                    <ResourceIdText value={session.authSessionId} />
                  </td>
                  <td className="px-4 py-3">
                    <EnumBadge value={session.state} />
                  </td>
                  <td className="px-4 py-3">
                    {formatTimestamp(session.lastSeenTime)}
                  </td>
                  <td className="px-4 py-3">
                    {formatTimestamp(session.expireTime)}
                  </td>
                  <td className="px-4 py-3">{clientLabel(session)}</td>
                  <td className="px-4 py-3">
                    {canRevoke ? (
                      <button
                        className="rounded px-2 py-1 text-red-700 hover:bg-red-50 dark:text-red-300 dark:hover:bg-red-950/50"
                        onClick={() => onRevokeSession(session.authSessionId)}
                      >
                        Revoke
                      </button>
                    ) : (
...
function OwnedSpaces({ spaces }: { spaces: SpaceInfo[] }) {
  return (
    ...
    >
      ...
      <Text intent="muted" size="sm" className="mt-1">
        Spaces where this principal is the owner.
      </Text>
      {spaces.length === 0 ? (
        <Text intent="muted" size="sm" className="mt-4">
          No owned spaces found.
        </Text>
      ) : (
        <div className="mt-4 overflow-hidden rounded-lg border border-slate-200 dark:border-slate-800">
          <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
            ...
            >
              <tr>
                ...
                <TableHead className="px-4 py-3">Space ID</TableHead>
                <TableHead className="px-4 py-3">State</TableHead>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
...
function ConfirmRevokeDialog({
  ...
  dialog: RevokeDialog;
  setDialog: (dialog: RevokeDialog) => void;
  loading: boolean;
  onCancel: () => void;
  onConfirm: () => void;
}) {
  const title =
    dialog.kind === "allSessions"
      ? "Revoke all principal sessions?"
      : dialog.kind === "session"
        ? "Revoke this principal session?"
        : "Revoke grant?";
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 px-4 backdrop-blur-sm dark:bg-slate-950/80">
      ...
      >
        <Text
          as="p"
          size="sm"
          className="font-medium uppercase tracking-[0.2em] text-red-400"
        >
          Revoke
        </Text>
        <H2
        ...
        <Text intent="muted" size="sm" className="mt-3">
          {dialog.kind === "session"
            ? `Session ${dialog.sessionId} will be revoked.`
            : dialog.kind === "allSessions"
              ? "The principal will be signed out from all active clients."
              : `${dialog.label} will be revoked.`}
        </Text>
        {(dialog.kind === "role" || dialog.kind === "capability") && (
          <label
            className={`mt-4 block text-sm font-medium ${themeClasses.text.parts.primaryLight} ${themeClasses.text.parts.darkPrimary}`}
          >
            Reason
            <textarea
              className={`mt-1 block min-h-20 w-full rounded-md border border-slate-300 ${themeClasses.surface.input} px-3 py-2 text-sm ${themeClasses.text.parts.primaryLight} ${themeClasses.focus.ring} dark:border-slate-700 ${themeClasses.text.parts.darkPrimary}`}
              value={dialog.reason}
              onChange={(event) =>
                setDialog({ ...dialog, reason: event.target.value })
              }
...
function accessScopeFromForm(form: GrantForm): AccessScopeInput {
  if (form.scopeType === "domain")
    return { type: "domain", spaceId: form.spaceId, domainId: form.domainId };
  if (form.scopeType === "space")
    return { type: "space", spaceId: form.spaceId };
  return { type: "system" };
}
...
function scopeKey(
  ...
  const type = (scope?.type || "system")
    .replace(/^ACCESS_SCOPE_TYPE_/i, "")
    .toLowerCase();
...
function clientLabel(session: PrincipalSessionInfo) {
  ...
  return (
    [client.name, client.version, client.platform, client.deviceLabel]
      .filter(Boolean)
      .join(" · ") || "Not reported"
  );
}
