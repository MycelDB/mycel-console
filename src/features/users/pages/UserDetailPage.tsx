...
export function UserDetailPage({
  ...
  return (
    <section className="space-y-6">
      ...
      {loading ? <Loading /> : user && activeTab === "overview" ? <><UserIdentity user={user} /><OwnedSpaces spaces={ownedSpaces} /><SemanticDiagnosticsNote ownedSpaceCount={ownedSpaces.length} /></> : null}
...
function SemanticDiagnosticsNote({ ownedSpaceCount }: { ownedSpaceCount: number }) {
  return <div className="rounded-xl border border-amber-200 bg-amber-50 p-6 dark:border-amber-900/70 dark:bg-amber-950/30"><Text as="h3" className="font-medium text-slate-900 dark:text-slate-100">Semantic diagnostics</Text><Text intent="muted" size="sm" className="mt-2 max-w-3xl text-slate-700 dark:text-slate-300">This panel captures what can be inferred for principal-reported semantic search issues today. The current Admin API can show this principal's state, sessions, owned spaces, space domains, semantic indexes, inference resources, and maintenance state. Effective roles and capabilities are available in the Roles & capabilities tab.</Text><div className="mt-4 grid gap-3 md:grid-cols-2"><DiagnosticItem status="pass" label="Principal state" detail="Shown above from AdminPrincipalService.GetPrincipal." /><DiagnosticItem status="pass" label="Principal auth sessions" detail="Shown in the Sessions tab from AdminPrincipalService.ListPrincipalSessions." /><DiagnosticItem status={ownedSpaceCount > 0 ? "pass" : "warn"} label="Owned spaces" detail={ownedSpaceCount > 0 ? `${ownedSpaceCount} owned space${ownedSpaceCount === 1 ? "" : "s"} found.` : "No owned spaces found for this user."} /><DiagnosticItem status="pass" label="Effective access" detail="Shown in the Roles & capabilities tab." /></div></div>;
}
...
function DiagnosticItem({ status, label, detail }: { status: "pass" | "warn" | "fail"; label: string; detail: string }) {
  ...
  return <div className="rounded-lg border border-amber-200 bg-white/70 p-4 dark:border-amber-900/60 dark:bg-slate-950/30"><Text as="p" size="sm" className="font-medium text-slate-900 dark:text-slate-100">{icon} {label}</Text><Text intent="muted" size="sm" className="mt-1 text-slate-700 dark:text-slate-300">{detail}</Text></div>;
}
